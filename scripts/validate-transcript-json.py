...
def excluded_source_ids(final, whisper, duration_ms):
    ...
    for item in excluded:
        ...
        require(reason in {"segment_not_object", "empty_text", "invalid_offsets", "non_positive_duration", "outside_source_duration"}, "排除分段原因无效")
        ...
        if reason == "segment_not_object":
            ...
            elif reason == "non_positive_duration":
                require(text and has_offsets and offsets["from"] >= offsets["to"], "排除分段原因不匹配")
            else:
                require(text and has_offsets and offsets["from"] >= duration_ms, "排除分段原因不匹配")
...
def main():
    ...
    excluded_ids = excluded_source_ids(final, whisper, duration_ms)
